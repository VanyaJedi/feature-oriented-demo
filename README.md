...
Добавьте товары и нажмите «Оформить заказ». `CheckoutService.placeOrder(OrderInput)`
проверяет входные данные, рассчитывает итог и имитирует запрос с задержкой 800 мс.
`useSummary` хранит товары и рассчитывает сумму; `useCheckout` хранит
`isSubmitting`, `error`, `orderId` и обрабатывает результат
через `try/catch/finally`, как в статье. Пока запрос выполняется, изменение корзины
и повторное оформление заблокированы. После успеха корзина очищается, появляется
номер `DEMO-…`; при ошибке товары сохраняются для повторной попытки.
Номера действуют только в текущем запуске демо, backend не используется.
